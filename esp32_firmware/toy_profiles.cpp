 ...
 * 主循环 / 串口协议 / server.py 均无需改动。跳蛋(TOY_PROFILE_JUMP_EGG)是
...
/* ============ 预留：男用设备 / 飞机杯类（伸缩·振动·气动等） ============
 * TODO(设备到货逆向后实现)：具体机型协议千差万别（UUID、包格式、是否需要
 * 握手/通知订阅、伸缩位置/速度、加热等各通道编码），禁止凭空臆造字节。
 * 实现前请参照 docs/飞机杯适配说明.md「接入步骤」与 buttplug.io/stpihkal
...
const ToyProfile toy_profiles[TOY_PROFILE_COUNT] = {
  /* TOY_PROFILE_JUMP_EGG —— 默认跳蛋，UUID/上限/报文与原固件完全一致 */
  {
...
const ToyProfile *toyActiveProfile(void) {
  ...
  if (id < 0 || id >= (int)TOY_PROFILE_COUNT) {
    id = TOY_PROFILE_JUMP_EGG; /* 越界回退默认跳蛋 */
  }
