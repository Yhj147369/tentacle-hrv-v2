 ...
 *
 * 背景：社区(amlyxts @ Koishi topic/13546 #7)请求"飞机杯兼容"。
 * 本框架把 ESP32 的"玩具通道"从写死常量参数化为设备类型 Profile：
 *   - JUMP_EGG（默认） = 现有跳蛋/振动类：UUID 0xFFE0/0xFFE1、
 *     报文 {0xA1,0x02,intensity,checksum,0xB3}、物理上限 60 —— 行为逐字节不变；
 *   - MALE_MST（预留） = 男用伸缩/多通道类（飞机杯）：具体 BLE 协议（UUID/
...
/* ============ 设备类型枚举 ============ */
typedef enum {
  TOY_PROFILE_JUMP_EGG = 0,   /* 默认：跳蛋/单振动通道（现协议，保持不变） */
  TOY_PROFILE_MALE_MST = 1,   /* 预留：男用伸缩/多通道设备（飞机杯类） */
  TOY_PROFILE_COUNT
} ToyProfileId;
