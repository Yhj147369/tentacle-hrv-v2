...
核心创新在 **输入端**：增加心率/HRV 实时监测和语音情绪识别，并扩展了随机事件、手动控制、自定义角色、数据可视化、氛围音效和 Web 配置后台。
...
- 🗣️ AI 语音朗读（TTS），勾选后自动朗读 AI 回复
- 🎲 随机事件与剧情分支系统，弹窗选择行动，影响剧情走向
- 🎛️ 手动控制模式，网页端直接发送 SET/STOP 指令，可调强度、时长、波形
- 🎭 自定义角色接口，玩家可粘贴自己的 SYSTEM_PROMPT，随时切换人设
- 💬 对话记忆增强，AI 能记住近期互动历史，使角色更连贯
- 📊 数据可视化面板，实时显示心率曲线和指令强度柱状图
- 🔊 声音氛围系统，根据心率自动生成不同频率背景音效
- ⚙️ Web 配置后台，网页端修改安全阈值、重新加载事件池
...
- 本机测试：`http://127.0.0.1:8080/?key=123456`  
- 平板访问：`https://你的域名/?key=123456`（需 HTTPS）

登录账号：`admin` / `123456`（或使用 URL 参数 `?key=123456` 跳过弹窗）
...
5. 勾选「AI语音朗读」可让 AI 回复自动朗读
6. 在「手动控制」区域，可拖动滑块选择强度、时长、波形，点击按钮发送 SET 或 STOP 指令
7. 随机事件触发时，会弹出选项按钮，需在倒计时内选择，超时将自动随机选择
8. 点击「编辑角色设定」可粘贴自定义 SYSTEM_PROMPT，保存后 AI 将使用新设定
9. 点击「管理后台」可修改心率阈值、强度上限等参数，并重新加载事件池
10. 勾选「氛围音效」可开启根据心率变化的背景音效
...
- 心率超过 130 会自动强制 STOP，请根据个人情况调整阈值（可在管理后台修改）
- 蓝牙玩具物理强度上限已在固件中限制为 60%
- 随机事件系统在心率偏高时会自动降级为放松事件，熔断时仅允许放松事件
...
### AI 语音朗读没有声音？
确保已安装 `edge-tts` 并联网，且勾选了「AI语音朗读」复选框。

### 管理后台修改的阈值没生效？
修改后立即生效于内存，但重启服务后会恢复默认值（除非写入配置文件）。目前 DLC9 是运行时修改，未持久化。如需持久化，可在 `server.py` 中将配置保存到文件。
...
- 原作者：[ra1nyxin/tentacle-monster-roleplay-esp32](https://github.com/ra1nyxin/tentacle-monster-roleplay-esp32)
- DeepSeek API
- Vosk 语音识别
- edge-tts
- Chart.js
