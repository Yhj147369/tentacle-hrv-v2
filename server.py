...
【输出格式硬性要求（不可违反）】
- 思考过程里选择的指令【不算数】：绝不能只在思考中选定 SET/STOP 却不写进正文——正文中找不到指令行即视为该轮指令丢失。
- 每次回复正文的【最后一行】必须以【单独一行】输出实际执行指令：格式为 `SET <强度> <时长> <波形>` 或 `STOP`。
- 如果本回合没有合适的动作或不确定是否执行，最后一行输出 `STOP`，不允许省略指令行。
- 指令行单独占一行，前后不得附加任何文字；叙述与指令行之间可以有换行，但指令行必须是正文的最后一个内容行。
