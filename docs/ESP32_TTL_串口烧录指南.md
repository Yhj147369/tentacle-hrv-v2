...
| 输出 | 出处 | 含义 |
|---|---|---|
| `HR:xx,IBI:yy` | `esp32_firmware.ino:223` | ✅ **最硬证据**：心率手环已连上并收到心率通知 |
| `✅ 手环已连接` | 行 238 | 心率手环连接成功 |
| `✅ 玩具已连接` | 行 256 | 体感反馈设备连接成功 |
| `⚠️ 手环连接失败，稍后重试` | 行 229 | 固件在跑，但手环没连上（BLE 未发现/未开机） |
| `⚠️ 玩具未连接，指令已记录` | 行 80 | 固件在跑，等你发指令 |
| `📤 发送: XX XX` | 行 93 | 收到了上位机下发的指令字节 |
| `⚠️ 未知波形: …` | 行 178 | 指令参数不合法 |
| `✅ 执行: 强度N 时长N秒 波形N` | 行 185 | 指令解析成功 |

**判定标准（按强弱排序）**：

1. **串口 115200 能读到任意一行上述文本** → 固件已烧入并正在运行。**注意：本固件在 `setup()` 里没有打印 “启动/版本” 之类的 banner**（只做 `Serial.begin` + `BLEDevice::init("ESP32_Bridge")` + 两次 BLE 扫描），所以**如果手环不在旁边，可能“看不到任何输出”却仍是正常运行**——这一点不要误判为失败。
2. 若手环已开机佩戴：应看到 `✅ 手环已连接` 然后持续 `HR:xx,IBI:yy`。**这是最可靠的运行证据**。
3. 想主动要输出：用串口发一条指令（项目支持的格式见 `handleSerialCommand`，如 `send 强度 时长 波形`），应得到 `✅ 执行: …` 或 `⚠️ 玩具未连接，指令已记录`。
